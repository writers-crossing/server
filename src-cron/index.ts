...
cron.schedule('0 0 * * *', async () => {
    ...
    for (const user of users) {
        if (user.dailyGoal && user.dailyGoal > 0) {
            await AwardXp.create({ discordId: user.discordId, xp: Math.round(user.wcDaily * config.awardXpPercentageGoal) })
        }
    }
...
cron.schedule('0 0 * * 1', async () => {
    ...
    for (const user of users) {
        if (user.weeklyGoal && user.weeklyGoal > 0) {
            await AwardXp.create({ discordId: user.discordId, xp: Math.round(user.wcWeekly * config.awardXpPercentageGoal) })
        }
    }
...
cron.schedule('0 0 1 * *', async () => {
    ...
    for (const user of users) {
        if (user.monthlyGoal && user.monthlyGoal > 0) {
            await AwardXp.create({ discordId: user.discordId, xp: Math.round(user.wcMonthly * config.awardXpPercentageGoal) })
        }
    }
...
cron.schedule('59 23 31 12 *', async () => {
    ...
    for (const user of users) {
        if (user.yearlyGoal && user.yearlyGoal > 0) {
            await AwardXp.create({ discordId: user.discordId, xp: Math.round(user.wcYearly * config.awardXpPercentageGoal) })
        }
    }
